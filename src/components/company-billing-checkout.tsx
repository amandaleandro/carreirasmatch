...
export function CompanyBillingCheckout({ packs, payerEmail }: { packs: Pack[]; payerEmail: string }) {
  ...
  if (pix) {
    return (
      <div className="space-y-3 rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 p-6 text-sm">
        ...
        <p className="text-xs text-neutral-500 dark:text-neutral-400">
          Após o pagamento, os {selected.credits} créditos entram automaticamente em alguns instantes.
        </p>
      </div>
    );
  }
        ...
        {packs.map((pack) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-lg font-bold mt-1">{formatBRL(pack.priceCents)}</p>
              <p className="text-xs text-neutral-500 mt-0.5">
                {formatBRL(Math.round(pack.priceCents / pack.credits))} por triagem
              </p>
            </button>
          );
        })}
      ...
      <div className="space-y-2">
        <Payment
          ...
          onSubmit={async (formData) => {
            setError(null);
            try {
              const res = await fetch("/api/empresa/billing/comprar", {
                method: "POST",
              ...
              if (data.pix) {
                ...
                setSuccess(`Pagamento aprovado! ${selected.credits} créditos adicionados.`);
                router.refresh();
