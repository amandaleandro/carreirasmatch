  ...
  return (
    <section className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
      <div className="rounded-3xl bg-gradient-to-br from-[#071827] via-[#101d42] to-[#312e81] p-5 text-white shadow-lg">
        <div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-widest text-blue-200">Sua evolução</p><h2 className="mt-1 text-2xl font-black">Nível {level}</h2><p className="mt-1 text-xs text-blue-100/75">{area ? `Treinando para ${area}` : "Complete seu perfil para personalizar ainda mais"}</p></div><div className="rounded-2xl bg-white/10 p-3"><Zap className="h-5 w-5 text-amber-300" /></div></div>
      ...
      <div className="rounded-3xl border border-amber-200 bg-amber-50 p-5 dark:border-amber-900/50 dark:bg-amber-950/20"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-widest text-amber-700 dark:text-amber-300">Desafio de hoje</p><h2 className="mt-1 text-lg font-black text-amber-950 dark:text-amber-100">{challenge.title}</h2></div><Target className="h-5 w-5 text-amber-500" /></div><p className="mt-2 text-xs leading-relaxed text-amber-900/70 dark:text-amber-200/70">Vale <strong>{challenge.reward} XP</strong> para manter sua evolução.</p><Link href={`/jogos/${challengePath}`} className="mt-4 inline-flex rounded-xl bg-amber-500 px-4 py-2.5 text-xs font-bold text-white hover:bg-amber-600">{challenge.completed ? "Revisar desafio" : "Começar agora"}</Link></div>
      <div className="rounded-3xl border border-neutral-200 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-900 lg:col-span-2"><div className="flex items-center justify-between gap-3"><h2 className="flex items-center gap-2 text-sm font-black"><Award className="h-4 w-4 text-violet-500" />Medalhas da sua jornada</h2><span className="text-[11px] text-neutral-400">Continue jogando para desbloquear</span></div><div className="mt-4 grid gap-2 sm:grid-cols-3 md:grid-cols-4">{(badges.length ? badges : [{ icon: "🔒", label: "Primeira partida", description: "Jogue para desbloquear" }]).map((badge) => <div key={badge.label} className="rounded-2xl bg-neutral-50 p-3 dark:bg-neutral-950"><span className="text-xl">{badge.icon}</span><p className="mt-1 text-xs font-bold">{badge.label}</p><p className="mt-0.5 text-[10px] text-neutral-500">{badge.description}</p></div>)}</div></div>
    </section>
  );
}
