...
import { Handshake, Mail, Lock, ArrowRight, ShieldCheck } from "lucide-react";
...
export default function PartnerLoginPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res = await signIn("partner-credentials", { email, password, redirect: false });
    ...
    >
      <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl shadow-slate-900/5 p-6 sm:p-8 space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="seuemail@parceiro.com"
          ...
          >
            <span>{loading ? "Entrando..." : "Entrar como parceiro"}</span>
        ...
        <div className="pt-4 border-t border-slate-100 dark:border-slate-800 text-center space-y-2">
          <p className="text-xs text-slate-600 dark:text-slate-400">
            Ainda não tem conta de parceiro?{" "}
            <Link href="/parceiro/cadastro" className="font-bold text-blue-600 dark:text-blue-400 hover:underline">
