...
export async function generateInterviewAnswerDraft(
  ...
  const modeInstructions =
    mode === "star"
      ? "Estruture a resposta no modelo STAR, deixando situa��o, a��o e resultado bem claros, sem soar engessado."
      : mode === "tecnica"
        ? "D� mais peso ao vocabul�rio t�cnico da �rea, aos conceitos usados e aos trade-offs, sem exagerar a experi�ncia."
        : "Prefira respostas curtas a m�dias, claras e naturais, como algu�m falaria numa entrevista real quando o tempo � curto.";

  const systemPrompt = `${INTERVIEWER_ROLE}
${BASE_RULES}
Sua tarefa é redigir um RASCUNHO de resposta para uma pergunta técnica ou comportamental de entrevista, usando somente o contexto fornecido pelo candidato e pela vaga.
Regras:
- Não invente experiência, emprego, certificação, ferramenta ou resultado que não esteja no contexto.
- Se o contexto não trouxer base suficiente, escreva uma resposta honesta, genérica e segura, indicando onde o candidato deve personalizar.
- Quando a pergunta pedir detalhes técnicos, inclua os conceitos corretos, mas sem overclaim.
${modeInstructions}
Formato de resposta:
{
  "answerDraft": string,
  "keyPoints": string[] (2 a 5 pontos que a pessoa deve manter ou adaptar),
  "cautionNotes": string[] (1 a 4 cuidados sobre o que não afirmar ou onde personalizar)
}`;
